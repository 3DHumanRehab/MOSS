...
# 确保上面给出的所有类和函数定义都在此环境中

# Step 1: 创建对偶四元数
# 假设我们有一个绕z轴旋转π/2的旋转和沿x轴平移5个单位的变换
rotation_angle = math.pi / 2
rotation_axis = [0, 0, 1]  # z轴
translation_vector = [5, 0, 0]  # x轴方向移动5

# 旋转四元数和平移四元数
rot_q = Quaternion.quaternionAboutAxis(rotation_angle, rotation_axis)
trans_q = Quaternion(0, *translation_vector)  # 转换为四元数形式

# 创建对偶四元数
dq = DualQuaternion(rot_q, trans_q)

# Step 2: 创建一个三维点
point = Vector(1, 1, 1)  # 一个简单的点

# Step 3: 使用对偶四元数变换这个点
# 提取平移和旋转
rotation = dq.getRotation
translation = dq.getTranslation

# 应用旋转和平移
rotated_point = rotation * point * rotation.conjugate  # 应用旋转
translated_point = rotated_point + translation  # 应用平移
