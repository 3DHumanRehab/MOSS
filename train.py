...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,file):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration % 300 == 0:
            ...
            test_image = image.permute(1,2,0)
            test_image[src_uv[0,:,1].type(torch.LongTensor), src_uv[0,:,0].type(torch.LongTensor)] = 1
            imageio.imwrite(f'vertex/img_{iteration}.png', (255*test_image).cpu().detach().numpy().astype(np.uint8))
            test_image = torch.zeros_like(image.permute(1,2,0))
            test_image[src_uv[0,:,1].type(torch.LongTensor), src_uv[0,:,0].type(torch.LongTensor)] = 1
            imageio.imwrite(f'vertex/img_point_{iteration}.png', (255*test_image).cpu().detach().numpy().astype(np.uint8))
        # Loss
        ...
        # loss = Ll1 + 0.3* (1.0 - ssim_loss) +  0.38* lpips_loss+  0.2 * nll_loss+ 0.11 * s3im_loss
...
if __name__ == "__main__":
    ...
    for name in name_list:
        print("Train on",name)
        file.write('\n'+"my_"+name+'\n')
        sys_list = ['-s', f'/home/zjlab1/dataset/ZJU_monocap/my_{name}', '--eval', '--exp_name', f'zju_mocap_refine/my_{name}_{file_name[:-4]}', '--motion_offset_flag', '--smpl_type', 'smpl', '--actor_gender', 'neutral', '--iterations', '2500']
