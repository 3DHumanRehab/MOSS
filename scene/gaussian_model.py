...
class Autoregression(nn.Module):
    def __init__(self,device='cuda'):
        ...
        for joint in range(self.num_joints):
            ...
            input_dim = embed_dim + num_parents * 3  # (passing (U, S, UV.T) for each parent to fc_pose - these have shapes (3x3), (3,), (3x3)
            self.fc_pose.append(nn.Sequential(nn.Linear(input_dim, embed_dim // 2),
                                              self.activation,
    ...
    # def forward_without(self,shape_params,glob):
        ...
        embed = self.activation(self.fc_embed(torch.cat([shape_params, glob,cam], dim=1)))  # (bsize, embed dim)
        batch_size = embed.shape[0]
        pose_F = torch.zeros(batch_size, self.num_joints, 3, device=self.device)  # (bsize, 23, 3, 3)
        for joint in range(self.num_joints):
            parents = self.parents_dict[joint]
            fc_joint = self.fc_pose[joint]
            if len(parents) > 0:
